function test_independent_learner_mpe_env(cap::Integer, nᵢ::Integer)
...
function test_independent_learner_pistonball_env(cap::Integer, nᵢ::Integer)
    e = PettingZooEnv("butterfly.pistonball_v6");
    m = MultiAgentManager(Dict(player =>
                    Agent(RandomPolicy(action_space(e, player)),
                        Trajectory(
                            container=CircularArraySARTTraces(
...
@testset "IndependentLearningAgents" begin
    @testset "Pettingzoo_MPE_independent" begin
        test_independent_learner_mpe_env(2, 100)
        test_independent_learner_mpe_env(40, 100)
        test_independent_learner_mpe_env(40, 25)
        test_independent_learner_mpe_env(400, 1_000)
    end
    @testset "Pettingzoo_Butterfly_pistonball_independent" begin
        #test_independent_learner_pistonball_env("butterfly.pistonball_v6", 400, 1_000, UInt8, (457, 120, 3,), Vector{Float32})
        #test_independent_learner_env("butterfly.pistonball_v6", 2, 100, Vector{Float32}, (457, 120, 3,), UInt8)
    end
end